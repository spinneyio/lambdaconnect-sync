(ns lambdaconnect-sync.test.pull-memory
  (:require [lambdaconnect-sync.test.basic-memory :refer [setup-basic-test-environment mobile-sync-config conn load-model-fixture slurp-fixture]]
...
(use-fixtures :once (partial setup-basic-test-environment (load-model-fixture "model1.xml") 
...
(deftest  test-pull
  (testing "pull;"
    (let [empty-db @conn
          entities-by-name (mp/entities-by-name (load-model-fixture "model1.xml"))
          input-json (slurp-fixture "fixtures.json")
      ...
      (is (= 0 (count (sync/get-changed-ids mobile-sync-config after-import "FIUser" (+ 1 (db/get-sync-revision mobile-sync-config after-import)) entities-by-name)))) ; 0 new users since 4 new users were created
      ...
      (is (= 1 (count (sync/get-changed-ids mobile-sync-config after-delete "FIUser" (+ 1 (db/get-sync-revision mobile-sync-config  after-import)) entities-by-name)))) ; 1 user modified (his game availability has been removed
      (is (= 0 (count (sync/get-changed-ids mobile-sync-config after-delete "FIUser" (+ 1 (db/get-sync-revision mobile-sync-config  after-delete)) entities-by-name)))) ; no changes, 0 entities changed
